...
class DS_PrefRepair
{
	...
	/**
	 * starting point for tool run from the command line
	 */
	public function run()
	{
		$this->msg( 'prefrepair:v1.0 - Copyright (C) 2018-2019 ServerPress, LLC. All Rights Reserved.', TRUE );
...
//			$username = getenv( 'USERNAME' );
			...
			if ( DS_OS_WINDOWS ) {
				$this->docdir = "C:\\Users\\{$username}\\Sites\\";
			} else if ( DS_OS_DARWIN ) {
	...
	/**
	 * Process a directory. Looks for a WP install and adds it to the list of DesktopServer known sites.
	 * @param string $dir Fully qualified path to the directory to process
	 */
	private function process_dir( $dir )
	{
		...
		if ( !class_exists( 'DS_ConfigFile', FALSE ) ) {
			for ( $libdir = __FILE__; ! file_exists( $libdir . DIRECTORY_SEPARATOR . 'htdocs' ); ) {
				$libdir = dirname ( $libdir );
			}
			$libdir .= DIRECTORY_SEPARATOR . 'htdocs' . DIRECTORY_SEPARATOR . 'classes' . DIRECTORY_SEPARATOR;

			if ( ! file_exists( $libdir ) )
				die( 'Unable to load dependent library: ' . $libdir );

			$loc = $libdir . 'gstring.php';
			require_once( $loc );
			$loc = $libdir . 'class-ds-config-file.php';
			require_once( $loc );
		}
...
//var_export($site_entry);
	...
	/**
	 * Checks to see if the DesktopServer application is running
	 * @return boolean TRUE if DesktopServer is running; otherwise FALSE
	 */
	...
	/**
	 * Loads data from the preferences file. Will construct a new one if it is not present.
	 */
	private function load_preferences()
	{
		if ( !file_exists( $this->pref_file ) ) {
			...
			$data = array(
				'version' => '3.9.4',						// this will be rewritten next time DesktopServer starts
				'edition' => 'Premium',						// this will be rewritten next time DesktopServer starts
				'webOwner' => '',
				'dbUser' => 'root',							// default credentials for MySQL/MariaDB
				'dbPass' => '',
				'browser' => $this->browser,
				'tld' => $this->tld,
				'ds-plugins' => new stdClass(),				// default to empty object #266
				'desktop' => $desktop,
...
//echo 'preferences=', var_export($this->pref, TRUE), PHP_EOL;
	...
	/**
	 * Saves data into the Preferences file
	 */
	private function save_preferences()
	{
		$json = json_encode( $this->pref, JSON_PRETTY_PRINT | JSON_FORCE_OBJECT );
...
//die('stop');
	...
	/**
	 * Displays a message if the -v parameter has been used
	 * @param string $msg The message to display
	 * @param boolean $output TRUE to override value of -v parameter
	 */
	...
	/**
	 * Displays usage information and then quits.
	 */
...
//		echo ' -d         Replace \'documents\' property with document-directory value', PHP_EOL;
